...
seam `cli/tests` monkeypatch to hand back a fake `Diagnos` that never
touches the network. `enrollment_progress` exists because enrollment is not
only `login`'s problem: `unlock()` is lazy (`sdk/README.md`), so *any*
...
ponto que `cli/tests` faz monkeypatch para devolver uma `Diagnos` falsa que
nunca toca a rede. `enrollment_progress` existe porque enrollment não é só
problema do `login`: `unlock()` é preguiçoso (`sdk/README.md`), então o
