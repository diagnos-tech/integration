...
| `IMGEXAM_HARDEN_PROCESS` | `DIAGNOS_HARDEN_PROCESS` | SDK (`transport/config.py`), Rust enclave (`sdk/native`) |
| `IMGEXAM_MEMORY_LOCK` | `DIAGNOS_MEMORY_LOCK` | Rust enclave (`sdk/native/src/locked/mod.rs`, `error.rs`) |
...
| `IMGEXAM_WORKSPACE_ID` | `DIAGNOS_WORKSPACE_ID` | deploy-time only — OpenBao policy bootstrap (`api/deploy/k8s/openbao/`, `api/deploy/compose/openbao/`) |
| `IMGEXAM_ACCOUNT_ID` | `DIAGNOS_ACCOUNT_ID` | deploy-time only — OpenBao policy bootstrap (`api/deploy/k8s/openbao/`, `api/deploy/compose/openbao/`) |
...
`sdk/src/diagnos/transport/config.py`) changed from `"imgexam"` to `"diagnos"`. It is still overridden by the
...
- The `imgexam-*-v1` HKDF/AAD labels (`sdk/src/diagnos/crypto/hkdf.py`, `hybrid.py`, `keys.py`,
  `sdk/native/src/hybrid.rs`) — for example `imgexam-sdk-hybrid-seal-v1`, `imgexam-patient-dek-v1`,
  `imgexam-drive-node-key-v1`.
- The OpenBao static seal key id `imgexam-static-v1` (`api/deploy/k8s/autounseal/static/seal.hcl`).
...
- [ ] **Leave untouched**: every `imgexam-*-v1` string in `crypto/`, `native/src/hybrid.rs`, `sdk/tests/vectors/`,
      and `imgexam-static-v1` in `api/deploy/k8s/autounseal/static/seal.hcl`.
...
live in secrets you don't want a blind rewrite near) or anything under `crypto/`, `native/src/`, `sdk/tests/vectors/`
