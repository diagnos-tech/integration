...
including a rekey. A regression in `sdk/native` therefore shows up as a
...
de tamanho de chunk, incluindo um rekey. Uma regressão em `sdk/native`
...
def _python_seal(x25519_public: bytes, mlkem768_public: bytes, plaintext: bytes, aad: str) -> EncryptedPayload:
    ...
    shares one line of code with `sdk/native/src/hybrid.rs`.
    ...
    enclave. Nada disso compartilha uma linha de código com `sdk/native/src/hybrid.rs`.
    """
