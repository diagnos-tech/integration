...
A REST facade (FastAPI) over the [`diagnos`](https://github.com/diagnos-tech/integration/tree/develop/sdk) SDK: one
...
> built from `api/Dockerfile` (below) is the supported way to run it today: it builds the SDK, CLI and API from
...
docker build -f api/Dockerfile -t diagnos-api .   # from the repository root
...
[Auto-unseal with OpenBao](https://github.com/diagnos-tech/integration/blob/develop/sdk/README.md#auto-unseal-with-openbao)
