...
def _require_tls_paths(source: Mapping[str, str]) -> tuple[str, str, str]:
    ...
    if not ca or not cert or not key:
        ...
        raise ApiConfigError(
            ...
            "a certificate/key pair to serve HTTPS with. See api/deploy/README.md. "
...
class ApiSettings:
    ...
    def from_env(env: Mapping[str, str] | None = None) -> ApiSettings:
        """🇺🇸 Reads the environment table documented in `api/deploy/README.md`.
        ...
        🇧🇷 Lê a tabela de ambiente documentada em `api/deploy/README.md`.
