...
| `a patient has a committed version and another one pending` | `workspace_id`, `security_group_id`, `document_id`, `version_id` | Uma versão `data` confirmada mais uma reserva pendente não expirada feita por **outra** conta (o cofre deixa uma conta substituir a própria), para uma reserva nova responder `409 DocumentVersionPending`. |
